...
I=id
A=None
import asyncio as L,argparse as a,datetime as G,logging as E,statistics as T,socket as O
C=a.ArgumentParser()
...
async def e():
	...
	for(J,Q)in c:
		...
		if not B.keepalive or C is A:
			...
			try:R=G.datetime.now();N=O.socket(O.AF_INET,O.SOCK_STREAM);N.connect((B.host,B.port));N.setblocking(Y);P,C=await L.open_connection(sock=N);F[I(C)]=N
			except W as K:E.error(f"{J}: Error open connection: {K}");H+=1;continue
		try:S=await L.wait_for(d(P,C,Q,J),B.timeout);D.append((G.datetime.now()-R).total_seconds()*1000);E.info(f"{J}: Success in {D[-1]} ms, data = {S}")
...
P,K=A,A
async def f():
	A=[L.create_task(e())for A in range(B.parallel)];global P,K;P=G.datetime.now()
	for C in A:await C
	K=G.datetime.now()
try:L.run(f())
except KeyboardInterrupt:K=G.datetime.now();pass
finally:
